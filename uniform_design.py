...
class table():
    ...
    def get_table(self):
        modulo_table = table.default_table(self)
        udt = table.uniform_design_table(self, modulo_table)
        udt_combination = table.udt_combination(self, udt)
        self.result = table.cd2_evaluate(self, udt, udt_combination)

    def cd2_evaluate(self, udt, udt_combination):
        """ CD2 """
        N = self.N
        S = self.S
...
#            for m in range(N):
#                for n in range(S):
#                    f2 *= (1 + 0.5*(abs(udt_cd2[m, n] - 0.5))
#                    - 0.5*(math.pow(abs(udt_cd2[m, n] - 0.5), 2)))
#                p2 += f2
#                f2 = 1
#            p2 = 2/N*p2
            ...
            for m in range(N):
                for k in range(N):
                    for n in range(S):
                        f3 *= (1 + 0.5*(abs(udt_cd2[m, n] - 0.5)) + 0.5*(abs(udt_cd2[k, n] - 0.5))
                        - 0.5*(abs(udt_cd2[m, n] - udt_cd2[k, n])))
                        f2 *= (1 + 0.5*(abs(udt_cd2[m, n] - 0.5))
                        - 0.5*(math.pow(abs(udt_cd2[m, n] - 0.5), 2)))
                    p2 += f2
                    f2 = 1
        ...
        return udt[:, udt_combination[np.argmin(record)]]

    def udt_combination(self, udt):
    ...
    def uniform_design_table(self, modulo_table):
        """ generate UDT """
        N = self.N
        col = list()
